...
export const ActorCard = ({
  ...
  return (
    ...
    >
      ...
      />
      <div className="absolute inset-0 bg-black mask-[linear-gradient(to_top,rgba(0,0,0,0.85)_30%,rgba(0,0,0,0.55)_65%,transparent_100%)]" />
      <div className="absolute inset-x-0 bottom-0 flex flex-col h-28 justify-start px-3 pt-11 items-center w-full pointer-events-none">
        <span className="text-gray-300 font-semibold text-center line-clamp-1 drop-shadow-md">
          {name}
        </span>
        <span className="text-gray-300 text-sm text-center line-clamp-2 drop-shadow-md">
          {character || "\u00A0"}
        </span>
      </div>
    </div>
